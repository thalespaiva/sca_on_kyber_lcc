...
#ifdef SIMULATE_ATTACK_ON_SHUFFLED_IMPLEMENTATION

  int N_ZEROS_TO_ATTACK_SHUFFLED_IMPLEMENTATION = 4;

  for (int i = 0; i < 32; i++) {
    buf[i] = 0xff;
  }
  for (int i = 0; i < N_ZEROS_TO_ATTACK_SHUFFLED_IMPLEMENTATION; i++) {
    int pos = -1;
    do {
      pos = (rand() % 256);
    } while (!((buf[pos / 8] >> (pos % 8)) & 1));
    buf[pos / 8] ^= (1 << (pos % 8));
  }
#endif
